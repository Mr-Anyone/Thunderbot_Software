...
double signedDistance(const Tbots::Rectangle &first, const Point &second)
{
    ...
    Vector abs_centre_to_point =
        Vector(std::abs(centre_to_point.x()), std::abs(centre_to_point.y()));
    Vector corner_to_point =
        abs_centre_to_point - (first.posXPosYCorner() - first.centre());
    Vector clamped_corner_to_point =
        Vector(std::max(corner_to_point.x(), 0.0), std::max(corner_to_point.y(), 0.0));
...
double signedDistance(const Tbots::Polygon &first, const Point &second)
{
    ...
    for (auto it = points.begin(), j = --points.end(); it != points.end(); j = it, ++it)
    {
        ...
        if ((second.y() >= it->y() && second.y() < j->y() &&
             e.x() * w.y() > e.y() * w.x()) ||
            (second.y() < it->y() && second.y() >= j->y() &&
             e.x() * w.y() <= e.y() * w.x()))
        {
            s *= -1;
        }
