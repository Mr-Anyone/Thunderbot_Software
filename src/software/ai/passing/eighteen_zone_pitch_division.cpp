...
EighteenZonePitchDivision::EighteenZonePitchDivision(const Field& field)
{
         ...
         pos_x += zone_width)
    {
        for (double pos_y = field.yLength() / 2; pos_y > -field.yLength() / 2;
             pos_y -= zone_height)
        {
            pitch_division_.emplace_back(Tbots::Rectangle(
                Point(pos_x, pos_y), Point(pos_x + zone_width, pos_y - zone_height)));
        }
    }

    zones_       = allValuesEighteenZoneId();
    field_lines_ = std::make_shared<Tbots::Rectangle>(field.fieldLines());
}

const Tbots::Rectangle& EighteenZonePitchDivision::getZone(EighteenZoneId zone_id) const
{
